...
if ( ! function_exists( 'st_set_script' ) ) {
	function st_set_script( $scripts, $handle, $src, $deps = [], $ver = false, $in_footer = false ) {
...
if ( ! function_exists( 'st_add_sidebar' ) ) {
	...
	function st_add_sidebar() {
		register_sidebar([
			...
			'before_title'  => '<label class="h4">',
			'after_title'   => '</label>',
			'before_widget' => '<div id="%1$s" class="sidebar-widget %2$s">',
			'after_widget'  => '</div>',
		]);
		if ( st()->get_option( 'option_style', 'modern' ) == 'classic' ) {
			register_sidebar([
			...
			register_sidebar([
				...
				'before_title'  => '<label class="shop-widget-title">',
				'after_title'   => '</label>',
...
if ( ! function_exists( 'st_add_scripts' ) ) {
	...
	function st_add_scripts() {
		...
		wp_register_script( 'moment.js', home_url() . '/wp-includes/js/dist/vendor/moment.min.js?ver=2.29.2', [ 'jquery' ], null, true );
		...
		wp_register_script( 'fullcalendar-lang', get_template_directory_uri() . '/js/fullcalendar-5.8.0/lib/locales-all.js', [ 'jquery', 'fullcalendar' ], null, true );
		...
		if ( is_page_template( 'template-user.php' ) ) {
			...
			if ( $check_enable_map_google === 'on' ) {
				...
				wp_register_script( 'mapboxv5', 'https://api.tiles.mapbox.com/mapbox-gl-js/v1.6.0/mapbox-gl.js', [], true, false );
				wp_register_script( 'mapboxv5-geocoder', 'https://api.mapbox.com/mapbox-gl-js/plugins/mapbox-gl-geocoder/v4.3.0/mapbox-gl-geocoder.min.js', [], true, false );
			...
			wp_register_script( 'moment.min', get_template_directory_uri() . '/js/moment.js', [ 'jquery' ], null, true );
			wp_register_script( 'prettify', get_template_directory_uri() . '/inc/plugins/ot-custom/fields/inventory/js/prettify.js', [ 'moment.min' ], null, true );
			wp_register_script( 'jquery.lang.gantt', get_template_directory_uri() . '/inc/plugins/ot-custom/fields/inventory/js/lang.js', [ 'jquery', 'prettify' ], null, true );
			wp_register_script( 'gantt-js', get_template_directory_uri() . '/inc/plugins/ot-custom/fields/inventory/js/jquery.fn.gantt.js', [ 'moment.min' ], null, true );
		...
		wp_enqueue_script( 'traveler', get_template_directory_uri() . '/dist/traveler.js', [ 'jquery' ], null, true );
		$check_enable_map_google = st()->get_option( 'st_googlemap_enabled' );
		if ( $check_enable_map_google === 'on' ) {
		...
		if ( is_page() && is_page_template( 'template-user.php' ) ) {
			...
			wp_localize_script('jquery', 'dashboard_params', [
				'theme_url'                  => get_template_directory_uri(),
				'site_url'                   => site_url(),
				'ajax_url'                   => admin_url( 'admin-ajax.php' ),
			...
			$sc       = STInput::get( 'sc' );
			$array_sc = [ 'create-hotel', 'edit-hotel', 'create-room', 'edit-room', 'create-tours', 'edit-tours', 'create-activity', 'edit-activity', 'create-rental', 'edit-rental', 'create-room-rental', 'edit-room-rental', 'create-cars', 'edit-cars', 'create-flight', 'edit-flight' ];
			...
			wp_register_script( 'st-partner-address_autocomplete', get_template_directory_uri() . '/v2/js/address_autocomplete.js', [ 'jquery', 'gmap-apiv3' ], false, true );
			wp_register_style( 'st-partner-address_autocomplete', get_template_directory_uri() . '/v2/css/address_autocomplete.css' );

			wp_register_script( 'st-partner-gmapv3', get_template_directory_uri() . '/v2/js/gmap3.min.js', [ 'jquery', 'gmap-apiv3' ], false, true );
			wp_register_script( 'st-partner-gmapv3-init', get_template_directory_uri() . '/v2/js/partner-map.js', [ 'st-partner-gmapv3' ], false, true );
			wp_register_script( 'init-mapbox-partner', get_template_directory_uri() . '/v2/js/mapbox/init-mapbox.js', [], false, true );
...
if ( ! function_exists( 'st_add_custom_css' ) ) {
	...
	function st_add_custom_css() {
		...
		} elseif ( New_Layout_Helper::isNewLayout() ) {
				$css .= st()->load_template( 'layouts/modern/custom_css', null, [ 'main_color_char' => $scheme ] );
		} else {
...
if ( ! function_exists( 'st_check_service_available' ) ) {
	...
	function st_check_service_available( $post_type = false ) {
		if ( $post_type ) {
			$st_theme_style = st()->get_option( 'st_theme_style' );
			if ( $st_theme_style !== 'classic' && ( $post_type == 'st_flight' ) ) {
				return false;
			}
			$disable_list = st()->get_option( 'list_disabled_feature' );
...
if ( ! function_exists( 'st_load_google_fonts_css' ) ) {
	...
	function st_load_google_fonts_css() {
		...
		if ( ! empty( $families ) ) {
			...
			wp_enqueue_style( 'st-google-fonts', esc_url( '//fonts.googleapis.com/css?family=' . implode( '', $families ) ) . $append, false, null );
		}
	}
}
...
function st_send_email_single_service() {
	...
	if ( $type_service === 'Hotel Room' ) {
		$hotel_parent = get_post_meta($id_service, 'room_parent', true);
		$hotel_name    = '';
		if (!empty($hotel_parent)) {
			$hotel_name = get_the_title( $hotel_parent );
		}
		$type_service .= '(' . $hotel_name . ')';
	}
	...
	if ( empty( $name_st ) || ( empty( $email_st ) ) || ( empty( $phone_st ) ) || ( empty( $content_st ) ) ) {
		...
		$multiple_to_recipients = [ $email_owl, $email_st, $admin_email ];
...
function post_thumbnail_html_func( $html, $post, $thumb, $size, $attr ) {
	if ( empty( $html ) ) {
		...
		return '<img src="https://experiences.reimagine.link/wp-content/uploads/2020/04/Bg-2.png"  class="' . $class . '"  alt="' . $alt . '"/>';
	}
	return $html;
}
function wp_get_attachment_image_attributes_func( $attr, $attachment, $size ) {
	if ( empty( $attr['src'] ) ) {
		$attr['src'] = 'https://experiences.reimagine.link/wp-content/uploads/2020/04/Bg-2.png';
	}
...
function wp_get_attachment_url_func( $url, $post_id ) {
	if ( empty( $post_id ) ) {
		$url = 'https://experiences.reimagine.link/wp-content/uploads/2020/04/Bg-2.png';
	}
...
function wp_get_attachment_image_srcc( $image, $attachment_id, $size, $icon ) {
	...
	if ( empty( $image[0] ) ) {
		$image    = [];
		$image[0] = 'https://experiences.reimagine.link/wp-content/uploads/2020/04/Bg-2.png';
		$image[1] = 900;
		$image[2] = 600;
	}
...
if ( class_exists( 'WPCF7_FormTag' ) ) {
	...
	function st_add_custom_contact() {
		...
		wpcf7_add_form_tag( 'url_service', 'wpcf7_url_service_shortcode_handler', true );
...
function wpcf7_url_service_shortcode_handler( $tag ) {
	if ( ! is_object( $tag ) ) {
		return '';
	}
	$name = $tag['name'];
	...
	$url_service = get_the_permalink();
	$html        = '<input type="hidden" name="' . $name . '" value="' . $url_service . '" />';
	return $html;
}
...
function st_rocket_deactivate_on_front_page() {
	if ( is_single() ) {
		add_filter( 'do_rocket_lazyload', '__return_false' );
	}
}
add_filter( 'wp', 'st_rocket_deactivate_on_front_page' );

// End lazy load img
function st_convert_characers_to_slug( $str, $options = [] ) {
...
if ( ! function_exists( 'st_notice_enqueue_css_js' ) ) {
	function st_notice_enqueue_css_js() {
		$pointer_name = 'install-plugin-library';
		$dismissed    = explode( ',', get_user_meta( get_current_user_id(), 'dismissed_wp_pointers', true ) );
		$content      = $content_demo  = $pointer_name_demo = $pointer_name_demo = '';
		if ( false === array_search( $pointer_name, $dismissed, true ) ) {
			$content  = '';
			$content .= '<h3>' . __( 'Important!', 'traveler' ) . '</h3>';
			$content .= '<p>' . __( 'You should install <a target="__blank" href="admin.php?page=st-vina-extensions">Traveler Layout Essential For Elementor Plugin</a> for more demo build by elementor .', 'traveler' ) . '</p>';

		}

		// Notice new demo
		$pointer_name_demo = 'install-new-demo-3-1-0';

		if ( false === array_search( $pointer_name_demo, $dismissed, true ) ) {
			$content_demo  = '';
			$content_demo .= '<h3>' . __( 'New Update!', 'traveler' ) . '</h3>';
			$content_demo .= '<p>' . __( 'Traveler version 3.1.0 has new demo <strong>SoloTour Elementor</strong>, new functions <strong><a target="_blank" href="https://travelerwp.com/new-version-3-1-0-new-update-solo-tour-demo-mega-menu-upsell-power-and-trust-badges/">Product Badge</strong>, <strong>Upsell Item</strong> and <strong>Mega Menu</strong></a> built by Elementor', 'traveler' ) . '</p>';
			if ( ! in_array( 'traveler-layout-essential-for-elementor/traveler-layout-essential-for-elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				$content_demo .= '<p>' . __( 'You need install <a target="__blank" href="admin.php?page=st-vina-extensions">Traveler Layout Essential For Elementor Plugin</a> for checking these demo.', 'traveler' ) . '</p>';
			}
		}
		if ( ! empty( $content ) || ! empty( $pointer_name ) || ! empty( $content_demo ) || ! empty( $pointer_name_demo ) ) {
			wp_enqueue_style( 'wp-pointer' );
			wp_enqueue_script( 'wp-pointer' );
			wp_enqueue_script( 'st-traveler-pointer', get_template_directory_uri() . '/js/admin/settings-pointer.js', [ 'wp-pointer' ], false );

			wp_localize_script(
				'st-traveler-pointer',
				'Traveler_Settings_Pointer',
				[
					'content'  => $content,
					'name'     => $pointer_name,
					'content2' => $content_demo,
					'name2'    => $pointer_name_demo,
				]
			);
		}
...
// Run file css/js
function st_check_single_style_mod() {
...
function st_listOfServiceSelect() {

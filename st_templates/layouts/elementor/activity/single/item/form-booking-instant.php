...
<?php
$ticketinghub_widget_id = get_post_meta(get_the_ID(), 'ticketinghub_widget_id', true);

if (!empty($ticketinghub_widget_id)) {
    // Show only the TicketingHub widget script
    echo '<script src="https://assets.ticketinghub.com/checkout.js" data-widget="' . esc_attr($ticketinghub_widget_id) . '"></script>';
} else {
    if ( !empty( $info_price[ 'discount' ] ) and $info_price[ 'discount' ] > 0 and $info_price[ 'price_new' ] > 0 ) { ?>
    ...
    <form id="form-booking-inpage" method="post" action="#booking-request" class="activity-booking-form form-has-guest-name">
        ...
        $end      = STInput::request( 'check_out', $start);
        ...
        <?php
        $enable_pickup = get_post_meta( get_the_ID(), 'enable_pickup', true );
        if ( $enable_pickup == 'on' ) :
            ?>
            <div class="form-group form-pickup">

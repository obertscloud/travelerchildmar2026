...
<div class="col-md-<?php echo esc_attr($col) ?> col-sm-6 col-xs-12 style_box has-matchHeight" itemscope itemtype="http://schema.org/TouristAttraction">
    ...
    <div class="thumb">
        ...
        <div class="thumb-caption">
            ...
            <!-- Place the reviews block here for best visibility -->
            <?php
            $reviews = TravelHelper::get_review($post_id, 2); // show up to 2 reviews
            if (!empty($reviews['review_list'])) {
                echo '<div class="st-tour--reviews">';
                foreach ($reviews['review_list'] as $review) {
                    echo '<div class="review-item">';
                    echo '<strong>' . esc_html($review['author_name']) . '</strong>: ';
                    echo '<p>' . esc_html(wp_trim_words($review['comment_content'], 15, '...')) . '</p>';
                    echo '</div>';
                }
                echo '</div>';
            }

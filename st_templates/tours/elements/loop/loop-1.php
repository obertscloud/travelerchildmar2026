...
<li <?php post_class('booking-item') ?> itemscope itemtype="http://schema.org/TouristAttraction">
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            ...
            <?php
            // Display up to 2 reviews for this tour just after the title
            $reviews = TravelHelper::get_review(get_the_ID(), 2);
            if (!empty($reviews['review_list'])) {
                echo '<div class="st-tour--reviews" style="margin-bottom:10px;">';
                foreach ($reviews['review_list'] as $review) {
                    echo '<div class="review-item" style="border:1px solid #eee; padding:4px 8px; margin-bottom:6px;">';
                    echo '<strong>' . esc_html($review['author_name']) . '</strong>: ';
                    echo '<span>' . esc_html(wp_trim_words($review['comment_content'], 15, '...')) . '</span>';
                    echo '</div>';
                }
                echo '</div>';
            }
